...
export default function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if (letterCount < 5) {
      setError(`Password contains only ${letterCount} letters. It requires a minimum of 5 alphabetic characters.`);
      return;
    }
  ...
  const styles = {
    ...
    card: {
      ...
      backgroundColor: isDarkMode ? 'rgba(10, 10, 10, 0.8)' : 'rgba(255, 255, 255, 0.9)',
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontWeight: '800', letterSpacing: '-0.02em', fontSize: '16px' }}>
            ACIKIWIR ACADEMY
          </span>
        </div>
        <button onClick={() => setIsDarkMode(!isDarkMode)} style={styles.toggleBtn}>
        ...
        <div style={styles.card}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <p style={{ margin: 0, fontSize: '14px', color: isDarkMode ? '#71717a' : '#64748b' }}>
              {isSignUpMode ? 'Sign up to make new account.' : 'Enter credentials to access website.'}
            </p>
          </div>
          ...
          <div style={styles.switchModeText}>
            {isSignUpMode ? 'Already signing up in the website?' : 'First time logging in this website?'}
      ...
      <footer style={{ textAlign: 'center', padding: '24px', fontSize: '11px', color: isDarkMode ? '#52525b' : '#94a3b8', letterSpacing: '0.05em' }}>
